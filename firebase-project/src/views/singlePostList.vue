  ...
  <div class="mb-0 mt-0 bg-gray-100 min-h-screen box-border">
    <div class="max-w-5xl mx-auto py-12 px-4 sm:px-6 lg:px-8">
      ...
      <div v-if="post" class="bg-white rounded-xl shadow-lg overflow-hidden">
        ...
        <div class="p-4">
          <h2 class="text-xl font-medium text-gray-900 tracking-normal font-medium">{{ post.title }}</h2>
...
export default {
  ...
  setup() {
    ...
    onMounted(async () => {
      try {
        const postId = route.params.postId; // Use the postId from the route parameters

        if (!postId) {
          throw new Error("Missing required param 'postId'");
        }
